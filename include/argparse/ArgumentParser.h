...
{
  public:
    explicit ArgumentParser(std::string_view program_name) : fProgramName(program_name)
    {
    }

    Argument &add_argument(std::string_view short_name, std::string_view long_name)
    {
    ...
    Argument &add_argument(std::string_view long_name)
    {
        return add_argument("", long_name);
    }
    Argument &add_argument(std::string_view name, Positional)
    {
    ...
    Argument &add_argument(std::initializer_list<std::string_view> names)
    {
        if (names.size() == 0)
    ...
    std::unordered_map<std::string, ArgumentValue> parse(int argc, char *argv[])
    {
  ...
  private:
    void parse_named_arguments(const std::vector<Token> &tokens, std::unordered_map<std::string, ArgumentValue> &result,
            ...
            if (arg->has_nargs())
            {
                ...
                if (arg->fixed_nargs())
                {
                    ...
                    if (collected.size() != expected)
                        throw ParseError("Expected " + std::to_string(expected) + " values after " +
    ...
    void parse_positional_arguments(const std::vector<Token> &tokens,
                                    std::unordered_map<std::string, ArgumentValue> &result,
                                    std::set<size_t> &consumed_indices)
    {
        ...
        for (Argument *arg : fPositionalArguments)
        {
            ...
            if (arg->has_nargs())
            {
                ...
                if (arg->fixed_nargs())
                {
                    ...
                    collected.insert(collected.end(), positionals.begin() + posIndex,
    ...
    void apply_defaults_and_validate(std::unordered_map<std::string, ArgumentValue> &result)
    {
        for (const auto &arg : fArguments)
        {
            std::string key = arg->long_name().starts_with("--") ? std::string(arg->long_name().substr(2))
    ...
    void print_help_and_exit() const
    {
        ...
        if (!fPositionalArguments.empty())
        {
            ...
            for (const auto *arg : fPositionalArguments)
            {
                std::cout << "  " << arg->long_name() << "\t" << arg->help() << "\n";
            }
            std::cout << std::endl;
        }
    ...
    std::vector<std::unique_ptr<Argument>> fArguments;
    std::unordered_map<std::string, Argument *> fArgumentLookup;
    std::vector<Argument *> fPositionalArguments;
};
